...
config = dict()
"""Train"""
config["display_iters"] = 95521
config["val_iters"] = 95521 * 2
...
config["preprocess_train_src"] = os.path.join(
    root_path, "dataset","preprocess", "pit_train_crs_dist6_angle90.p"
)
config["preprocess_train_tar"] = os.path.join(
    root_path, "dataset","preprocess", "mia_train_crs_dist6_angle90.p"
)
config["preprocess_val_src"] = os.path.join(
    root_path,"dataset", "preprocess", "pit_val_crs_dist6_angle90.p"
)

config["preprocess_val_tar"] = os.path.join(
    root_path,"dataset", "preprocess", "mia_val_crs_dist6_angle90.p"
)
config["preprocess_val"] = os.path.join(
    root_path,"dataset", "preprocess", "mia_val_crs_dist6_angle90.p"
)

config['preprocess_test'] = os.path.join(root_path, "dataset",'preprocess', 'mia_test_test.p')
...
class Net(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        if data['type'] == 'val' or data['type'] == 'test':
            out, _, _ = self.feature_osw(data["feats"], data["ctrs"], data["graph"], rot, orig, flag=None,
                                                       data_type=data['type'])
            return out
        out_all = {}
        out, agents_m, agents_a = self.feature_osw(data["feats"], data["ctrs"], data["graph"], rot, orig, flag=None, data_type=data['type'])
        out_sw, agents_m_sw, agents_a_sw = self.feature_osw(data["feats_s"], data["ctrs_s"], data["graph"], rot, orig, flag='sw', data_type=data['type'])
        out_ws, agents_m_ws, agents_a_ws = self.feature_osw(data["feats_w"], data["ctrs_w"], data["graph"],  rot, orig, flag='ws', data_type=data['type'])
        out_all['out'], out_all['out_sw'], out_all['out_ws'] = out, out_sw, out_ws
        out_all['agents_m'], out_all['agents_m_sw'], out_all['agents_m_ws'] = agents_m, agents_m_sw, agents_m_ws
        out_all['agents_a'], out_all['agents_a_sw'], out_all['agents_a_ws'] = agents_a, agents_a_sw, agents_a_ws
        return out_all


    def feature_osw(self, feats, ctrs, graph, rot, orig, flag=None, data_type=None):
        ...
        graph = graph_gather(to_long(gpu(graph)))
        nodes, node_idcs, node_ctrs = self.map_net(graph)  # nodes --> tensor, node_idcs, node_ctrs -->list
        ...
        if flag is not None:
            ...
            for i in range(len(node_idcs)):
                ctrs_i = node_ctrs[i]
                aug_number = ctrs_i.shape[0] - int(self.config['aug_pct_graph'] * (ctrs_i.shape[0]))
                if flag == 'ws':
                    # construct weak actor, strong map
                    aug_idx = torch.argsort(torch.linalg.norm(ctrs_i, axis=1))[-aug_number:]
                elif flag == 'sw':
                    # construct strong actor, weak map
                    aug_idx = torch.argsort(torch.linalg.norm(ctrs_i, axis=1))[:aug_number]
                nodes_i = nodes[node_idcs[i]][aug_idx]
                ctrs_i = ctrs_i[aug_idx]
                node_idcs_aug.append(range(count, count + aug_number))
                count += aug_number
                nodes_aug.append(nodes_i)
                ctrs_aug.append(ctrs_i)
            nodes = torch.cat(nodes_aug)
            node_idcs = node_idcs_aug
            node_ctrs = ctrs_aug
        actors = self.m2a(actors, actor_idcs, actor_ctrs, nodes, node_idcs, node_ctrs)  # dui qi
        agent_idcx = torch.cat([x[0].reshape(1) for x in actor_idcs])
        agents_m = actors[agent_idcx]
        actors = self.a2a(actors, actor_idcs, actor_ctrs)  # dui qi
        agents_a = actors[agent_idcx]
        ...
        if flag is not None or data_type == 'tar':
            out['reg'] = [x[0:1] for x in out['reg']]
            out['cls'] = [x[0:1] for x in out['cls']]

        return out, agents_m, agents_a
...
class Loss(nn.Module):
    def __init__(self, config):
        ...
        self.aug_loss = nn.SmoothL1Loss(reduction="sum")

    def forward(self, out: Dict, data: Dict) -> Dict:
        data['agt_gt_preds'] = [x[0:1] for x in data["gt_preds"]]
        data['agt_has_preds'] = [x[0:1] for x in data["has_preds"]]
        if data['type'] == 'val' or data['type'] == 'test':
            ...
            loss_out = self.pred_loss(out['out'], gpu(data["gt_preds"]), gpu(data["has_preds"]))
            loss_out["loss"] = loss_out["cls_loss"] / (
                loss_out["num_cls"] + 1e-10
            ) + loss_out["reg_loss"] / (loss_out["num_reg"] + 1e-10)

            loss_out_sw = self.pred_loss(out['out_sw'], gpu(data["agt_gt_preds"]), gpu(data["agt_has_preds"]))
            loss_out_sw["loss"] = (loss_out_sw["cls_loss"] / (
                loss_out_sw["num_cls"] + 1e-10
            ) + loss_out_sw["reg_loss"] / (loss_out_sw["num_reg"] + 1e-10))*0.001

            loss_out_ws = self.pred_loss(out['out_ws'], gpu(data["agt_gt_preds"]), gpu(data["agt_has_preds"]))
            loss_out_ws["loss"] = (loss_out_ws["cls_loss"] / (
                loss_out_ws["num_cls"] + 1e-10
            ) + loss_out_ws["reg_loss"] / (loss_out_ws["num_reg"] + 1e-10))*0.001
            return loss_out, loss_out_sw, loss_out_ws
        elif data['type'] == 'tar':
            loss_aug_out = self.aug_loss(torch.cat(out['out_sw']['reg']), torch.cat(out['out']['reg'])) \
                       + self.aug_loss(torch.cat(out['out_sw']['cls']), torch.cat(out['out']['cls'])) \
                       + self.aug_loss(torch.cat(out['out_ws']['reg']), torch.cat(out['out']['reg'])) \
                       + self.aug_loss(torch.cat(out['out_ws']['cls']), torch.cat(out['out']['cls']))

            loss_aug_feature = self.aug_loss(out['agents_m_sw'], out['agents_m'])\
                               + self.aug_loss(out['agents_m_ws'], out['agents_m'])
            return (loss_aug_out + loss_aug_feature)*0.001
...
class PostProcess(nn.Module):
    ...
    def display(self, metrics, dt, epoch, lr=None, flag=None):
        """Every display-iters print training/val information"""
        if lr is not None:
            print("Epoch %3.3f, lr %.5f, time %3.2f" % (epoch, lr, dt))
        else:
            print(
                "************************* %s Validation, time %3.2f *************************"
                % (flag, dt)
            )
