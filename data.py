...
class ArgoDataset(Dataset):
    def __init__(self, split, config, train=None):
        self.config = config
        self.train = train
        
        if 'preprocess' in config and config['preprocess']:
            if train == 't_s':
                self.split = np.load(self.config['preprocess_train_src'], allow_pickle=True)
            elif train == 't_t':
                self.split = np.load(self.config['preprocess_train_tar'], allow_pickle=True)
            elif train == 'f_s':
                self.split = np.load(self.config['preprocess_val_src'], allow_pickle=True)
            elif train == 'f_t':
                self.split = np.load(self.config['preprocess_val_tar'], allow_pickle=True)
        else:
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                new_data = dict()
                for key in ['idx', 'city', 'orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'ctrs', 'graph']:
                    if key in data:
                        new_data[key] = ref_copy(data[key])
                data = new_data
