...
def modify(config, data_loader, save):
    ...
    for i in range(len(store)):
        if store[i]['city'] == 'PIT':
            list_pit.append(store[i])
        if store[i]['city'] == 'MIA':
            list_mia.append(store[i])

    f = open(os.path.join(root_path, 'dataset','preprocess', 'mia_'+save.split('/')[-1]), 'wb')
    pickle.dump(list_mia, f, protocol=pickle.HIGHEST_PROTOCOL)
    f.close()

    f = open(os.path.join(root_path, 'dataset','preprocess', 'pit_'+save.split('/')[-1]), 'wb')
    pickle.dump(list_pit, f, protocol=pickle.HIGHEST_PROTOCOL)
    f.close()
