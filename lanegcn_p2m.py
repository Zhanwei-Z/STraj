...
config['preprocess_test'] = os.path.join(root_path, "dataset",'preprocess', 'mia_test_test_small.p')
...
class Net(nn.Module):
    ...
    def forward(self, data: Dict):
        ...
        if data.get('type', None) == 'tar_eval':
            out['reg_c'] = [x[0:1].detach().cpu().numpy() for x in out['reg']]
            out['cls_c'] = [x[0:1].detach().cpu().numpy() for x in out['cls']]
        if data.get('type', None) == 'tar_train':
            cls_idx = [x[0].argmax() for x in out['cls']]
            reg_batch = [torch.unsqueeze(x[0][y], 0) for x, y in zip(out['reg'], cls_idx)]
            reg_batch = torch.cat(reg_batch, dim=0)
            return reg_batch, agents_a
        ...
        if data.get('type', None) == 'src':
            gt_preds, has_preds = gpu(data["gt_preds"]), gpu(data["has_preds"])
            gt_preds = torch.cat([x[0:1] for x in gt_preds], 0)
            has_preds = torch.cat([x[0:1] for x in has_preds], 0)
            for i in range(len(gt_preds)):
                gt_preds[i] = torch.matmul(rot[i], (gt_preds[i] - orig[i].reshape(-1, 2)).T).T
            out['gt_preds'] = gt_preds
            out['agents_a'] = agents_a
        return out
...
class ContraLoss(nn.Module):
    ...
    def single_loss(self, anchor, positive, negative, t):  # (1,dim), (1,dim), (N, dim)
        pos = torch.exp(torch.cosine_similarity(anchor, positive, dim=1)/t)
        neg = torch.exp(torch.cosine_similarity(anchor, negative, dim=1)/t).sum()
        loss = -torch.log(pos/(pos+neg+1e-10))
        return loss

    def forward(self, prediction1, agents1, prediction2=None, agents2=None):
        min_dist = self.config['min_dist']
        max_dist = self.config['max_dist']
        if prediction2 is None:  # intra
            t = self.config['t_intra']
            x1 = torch.cat([torch.unsqueeze(torch.cdist(prediction1[:, i], prediction1[:, i], p=2), dim=1)
                            for i in range(prediction1.shape[1])], dim=1).mean(dim=1)
            for i in range(x1.shape[0]):
                x1[i, i] = min_dist + max_dist
            mask_min = x1[torch.arange(len(prediction1)), x1.argmin(dim=1)] < min_dist
            if mask_min.sum() == 0:
                return 0
            mask_max = x1[torch.arange(len(prediction1)), x1.argmax(dim=1)] > max_dist
            if mask_max.sum() == 0:
                return 0
            positive_pair = agents1[x1.argmin(dim=1)]
            negative_pairs = [agents1[i > max_dist] for i in x1]
            loss = [self.single_loss(agents1[i:i+1], positive_pair[i:i+1], negative_pairs[i], t)
                    if (mask_min[i] and mask_max[i]) else torch.tensor([0]).to(x1.device) for i in range(len(agents1))]
            loss = torch.cat(loss).sum()
            return loss
        else:
            t = self.config['t_inter']
            x1 = torch.cat([torch.unsqueeze(torch.cdist(prediction1[:, i], prediction2[:, i], p=2), dim=1)
                            for i in range(prediction1.shape[1])], dim=1).mean(dim=1)
            mask_min = x1[torch.arange(len(prediction1)), x1.argmin(dim=1)] < min_dist
            if mask_min.sum() == 0:
                return 0
            ...
            if mask_max.sum() == 0:
                return 0
            positive_pair = agents2[x1.argmin(dim=1)]
            negative_pairs = [agents2[i > max_dist] for i in x1]
            loss = [self.single_loss(agents1[i:i + 1], positive_pair[i:i + 1], negative_pairs[i], t)
                    if (mask_min[i] and mask_max[i]) else torch.tensor([0]).to(x1.device) for i in range(len(agents1))]
            loss = torch.cat(loss).sum()
            return loss
...
class Loss(nn.Module):
    def __init__(self, config):
        ...
        self.ps_loss = nn.SmoothL1Loss(reduction="sum")
    def forward(self, out: Dict, data: Dict, mem_ps=None, threshold_ps=None) -> Dict:
        if data.get('type', None) == 'tar_train':
            cls = torch.tensor([mem_ps[x]['cls'] if type(mem_ps[x]['cls']) != list else -10000 for x in data['idx']]).to(out.device) > threshold_ps
            flag = torch.tensor([mem_ps[x]['flag'] for x in data['idx']]).to(out.device)*cls
            if (flag.sum() == 0) or (threshold_ps is None):
                return 0
            cos_e = torch.tensor([mem_ps[x]['cos_e'] for x in data['idx']]).to(out.device)[flag]
            l2_e = torch.tensor([mem_ps[x]['l2_e'] for x in data['idx']]).to(out.device)[flag]
            reg_ps = torch.tensor(np.concatenate([mem_ps[x]['reg'][0:1] for x in data['idx']], axis=0)).to(out.device)[flag]
            weight = torch.exp(-((cos_e-1)**2+(l2_e/1.5)**2)/2)
            ps_loss = torch.cat([self.ps_loss(x, y).reshape(-1) for x, y in zip(out[flag], reg_ps)], dim=0)*weight
            return ps_loss.sum()/(out[flag].shape[0]*out[flag].shape[1]*out[flag].shape[2])
...
def pred_metrics(preds, gt_preds, has_preds):
    ...
    # print(err.shape, err[:, 0].mean().shape, err[:, 0].shape, 88888)
    ade1 = err[:, 0].mean()
    fde1 = err[:, 0, -1].mean()
...
def get_model():
    ...
    contra_loss = ContraLoss(config).cuda()

    return config, ArgoDataset, collate_fn, net, loss, post_process, opt, contra_loss
